...
// upload 폴더를 생성하는 명령, 만약 현재 위치에 해당 폴더가 없다면
...
const upload = multer({
    ...
    storage : multer.diskStorage({
        destination(req, file, done){
            ...
            // 첫번째 인수 null 은 현재 파일 (file)의 경로와 이름 그대로 사용.
            // (변경 및 추가 없음)
        },
        filename(req, file, done) {
            const ext = path.extname(file.originalname);  // 확장자 추출
            // 파일이름 + 오늘 날짜 (밀리초) + 추출된 확장자
            done(null, path.basename(file.originalname, ext) + Date.now() + ext);
            // abc.jpg - > abc123456483.jpg
            // 업로드 파일명이 같은 경우 처리할 객체가 없고, 위와 같은 방법으로
            // 파일명의 충돌을 방지합니다. (오늘날짜의 밀리초 값)
        },
    }),
...
// 'image' 업로드에서 전송된 폼데이터 중 파일 선택 태크의 이름과 일치해야 합니다.
